...
def main():
    ...
    #These are the ranges we will include, and these
    #lists are referenced later in the code

    x_array_indices = range(100,161)
    y_array_indices = range(9,35)
    z_array_indices = range(50)

    #Here we create our figure object, and add an axes
    #object to the figure, note the projection='3d'

    fig = plt.figure()
    ax = fig.add_subplot(111,projection='3d')

    #These are lists that will separate the points in
    #the array where the value is positive, vs negative
    pos_x=[]
    pos_y=[]
    pos_z=[]
    neg_x=[]
    ...
    for i_y in range(len(y_array_indices)):
        for i_z in [c_sparse*i for i in range(len(z_array_indices)/c_sparse)]:
            for i_x in [c_sparse*i for i in range(len(x_array_indices)/c_sparse)]:
                #We want to get the average value of the array elements
                #in the range that we'll represent with just a single point
    ...
    ax.set_xlim3d(np.log10(x_array[x_array_indices[0]]),
                  np.log10(x_array[x_array_indices[-1]]))
    ...
    for cf in [f,file_x,file_y,file_z]:
        cf.close()
